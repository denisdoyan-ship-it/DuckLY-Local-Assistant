    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if title and content:
                return f"\n\n📌\n{title}\n📘\n{content}\n\n"[:4096] + "...(ОБРЕЗАНО по maximal_length: 4096 characters)."
...
def run_command(cmd):
    try:
        ...
        if output and not error:
            return f"✅ Команда выполнена:\n{output[:4096] + "...(ОБРЕЗАНО по maximal_length: 4096 characters)."}"
        elif error:
...
class ChatApplication:
    ...
    def parse_and_call_tool(self, text):
        ...
        allowed_funcs = [
            ...
            "write_file", "edit_file", "create_dir", "delete_file", "delete_dir", "run_command"
        ]

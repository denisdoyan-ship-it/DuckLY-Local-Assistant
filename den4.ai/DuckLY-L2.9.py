    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if title and content:
                return f"\n\n📌\n{title}\n📘\n{content}\n\n"[:8192] + "...(ОБРЕЗАНО по maximal_length: 8192 characters)."
...
def delete_dir(path):
    ...
    if not path.exists() or not path.is_dir():
        return f"❌ Путь `{path}` не существует или не является директорией"
    ...
    for subdir in sorted(path.glob('**/*'), reverse=True):
        if subdir.is_dir():
            subdir.rmdir()

    path.rmdir()
    return f"🗑️ Папка `{path}` успешно удалена."
    ...
    try:
        ...
        def read_stream(stream, target_list):
            while True:
                ...
                decoded = decode_safely(line)
                target_list.append(decoded)

        t1 = threading.Thread(target=read_stream, args=(proc.stdout, output_lines), daemon=True)
        t2 = threading.Thread(target=read_stream, args=(proc.stderr, error_lines), daemon=True)
        t1.start()
        t2.start()
        ...
        if output and not error:
            return f"✅ Команда выполнена:\n{output[:max_len]}" + "...(ОБРЕЗАНО по maximal_length: 8192 characters)."
        elif error:
            return f"❌ Ошибка выполнения:\n{error[:max_len]}" + "...(ОБРЕЗАНО по maximal_length: 8192 characters)."
        else:
            return "✅ Команда выполнена (вывод пуст)."

    except subprocess.TimeoutExpired:
        proc.kill()
        return "❌ Ошибка: команда превысила лимит времени (60 сек)."
    except Exception as e:
...
class ChatApplication:
    ...
    def __init__(self, root):
        ...
        system_message = {
            "role": "system",
            "content": """
            Вы — DuckLY, локальный LLM-помощник и агент, разработанный den4.ai. Вы работаете автономно и помогаете пользователю в реальном времени.
            ...
            7. Если инструмент вернул ошибку — попробуйте пояснить, что не получилось, либо же попробуйте ещё раз с более правильными аргументами, если нужно.

            🔹 ДОСТУПНЫЕ ИНСТРУМЕНТЫ:
            - search_web("запрос") — поиск в интернете
            - get_cwd() — получить текущую директорию
            - list_files("путь") — список файлов (по умолчанию: ".")
            ...
            - run_command("команда") — выполнить команду в терминале PowerShell (Windows)
            ...
            - Вызов инструмента за инструментом (в разных сообщениях): если требуется/прошлый инструмент провалился.
            - Вызов инструмента, только если есть явная причина

            ❌ Нельзя:
            - Два одинаковых инструмента с одинаковыми аргументами подряд
            - Два вызова в одном сообщении
            - Вызов без причины
            - Перевод вызова на другой язык
            - Комментарии типа "Я воспользуюсь инструментом..."
            ...
            - Если действие провалилось — не повторяйте его, а предложите другое решение.
            """.strip(),
        }
        ...
        if state:
            ...
            if not has_system_prompt:
                restored_messages = [system_message] + [m for m in restored_messages if m["role"] != "system"]
            else:
                filtered = [m for m in restored_messages if m["role"] != "system"]
                restored_messages = [system_message] + filtered
    ...
    def _create_bubble(self, message, sender):
        ...
        label.bind("<Configure>", lambda e: self.update_wraplength(label))
    ...
    def update_wraplength(self, label):
        try:
            current_width = self.chat_canvas.winfo_width() - 40
            new_wrap = max(300, current_width - self.bubble_pad_x * 2 - 20)
            if label.winfo_width() > 1 and label['wraplength'] != new_wrap:
    ...
    def process_message(self, user_input, audio_b64=""):
        ...
        if current_assistant_message.strip():
            ...
            self.parse_and_call_tool(current_assistant_message)
    ...
    def parse_and_call_tool(self, text):
        ...
        allowed_funcs = [
            "search_web", "get_cwd", "list_files", "read_file",
            "write_file", "edit_file", "create_dir", "delete_file",
            "delete_dir", "run_command"
        ]
        ...
        args = [codecs.decode(arg, 'unicode_escape') for arg in arg_matches]

        expected_args = {
            "search_web": 1, "read_file": 1, "list_files": 1, "get_cwd": 0,
            "write_file": 2, "edit_file": 3, "create_dir": 1,
            "delete_file": 1, "delete_dir": 1, "run_command": 1
        }
        if len(args) != expected_args.get(func_name, 0):
            return None
        ...
        self.ask_for_approval(call_line)
        return None
    
    def ask_for_approval(self, call_line):
        frame = tk.Frame(self.chat_inner, bg=self.bg)
        frame.pack(fill=tk.X, pady=8, padx=self.hpad - 2)

        bubble = tk.Frame(frame, bg=self.system_bg, padx=16, pady=12)
        bubble.pack(anchor="w", fill=tk.X)

        header = tk.Label(
            bubble,
            text="⚠️ Подтвердите действие",
            bg=self.system_bg,
            fg="#ffffff",
            font=("Segoe UI", self.bubble_header_size, "bold")
        )
        ...
        label.pack(anchor="w", pady=(8, 12))

        btn_frame = tk.Frame(bubble, bg=self.system_bg)
        ...
        approve_btn = Button(
            ...
            pady=6,
            cursor="hand2",
            command=lambda: self.execute_pending_action(True, frame)
        )
        ...
        deny_btn = Button(
            ...
            pady=6,
            cursor="hand2",
            command=lambda: self.execute_pending_action(False, frame)
        )
        deny_btn.pack(side=tk.LEFT)

        self._scroll_to_bottom()

    def execute_pending_action(self, approved, approval_frame):
        approval_frame.destroy()
        global pending_action
    ...
    def _execute_in_background(self, approved):
        global pending_action

        if not pending_action:
            return

        func_name = pending_action["func_name"]
        kwargs = pending_action["kwargs"]

        if approved:
            self.root.after(0, lambda: self.display_message(f"🔧 Выполняется: `{pending_action['call_line']}`", "system"))
            result = self.call_tool(func_name, kwargs)
            system_msg = {
                "role": "system",
                "content": f"[{func_name.upper()}]: {result}"
            }
            messages_list.append(system_msg)
            if len(messages_list) > MAX_HISTORY + 1:
                messages_list[:] = [messages_list[0]] + messages_list[-MAX_HISTORY:]

            self.root.after(0, lambda: self.process_message(""))
        else:
            self.root.after(0, lambda: self.display_message("❌ Действие отменено пользователем.", "system"))
            system_msg = {
                "role": "system",
                "content": f"[USER_DENIED_ACTION]: действие `{func_name}` было отменено пользователем."
            }
            messages_list.append(system_msg)
            if len(messages_list) > MAX_HISTORY + 1:
                messages_list[:] = [messages_list[0]] + messages_list[-MAX_HISTORY:]
        ...
        self.root.after(0, lambda: self.set_typing(False))


    def _on_tool_result(self, result, func_name):
        system_msg = {
            "role": "system",
            "content": f"[{func_name.upper()}]: {result}"
        }
    ...
    def speak_text(self, text):
        ...
        if ($selectedVoice) {{
            ...
            Write-Host "Голос $voiceName не найден."
        }}

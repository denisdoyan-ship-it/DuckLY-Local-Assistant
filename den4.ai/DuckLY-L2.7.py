...
def save_state():
    ...
    if len(messages_list) > 1:
        recent_messages = [messages_list[0]] + messages_list[-MAX_HISTORY:]
    else:
    ...
    try:
        ...
        print(f"[DEBUG] Состояние сохранено: {len(recent_messages)} сообщений, модель={model}")
    except Exception as e:
        print(f"[DEBUG] Не удалось сохранить состояние: {e}")
...
def load_state():
    ...
    try:
        ...
        print(f"[DEBUG] Ошибка загрузки состояния: {e}")
        return None
...
def load_api_key():
    try:
        ...
        print(f"[DEBUG] Ошибка чтения api_key.txt: {e}")
    return ""
...
def save_api_key(key):
    try:
        ...
        print(f"[DEBUG] Не удалось сохранить api_key.txt: {e}")
...
class ChatApplication:
    ...
    def __init__(self, root):
        ...
        if state:
            ...
            for msg in restored_messages[1:]:
                self.display_message(msg.get("content", ""), "user" if msg["role"] == "user" else "assistant")
        else:
    ...
    def send_message_from_voice(self, base64):
        ...
        assistant_label = self._create_bubble("", "assistant")
        self.current_assistant_bubble = {"label": assistant_label}
        threading.Thread(target=self.process_message, args=("", base64,), daemon=True).start()
